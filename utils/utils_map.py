...
def get_map(MINOVERLAP, draw_plot, path = './map_out_IRDST_992_2_1'):
    ...
    for txt_file in ground_truth_files_list:
        ...
        file_id     = os.path.basename(os.path.normpath(file_id))       #获取gt的txt名称
        temp_path   = os.path.join(DR_PATH, (file_id + ".txt"))         #定位到detection_results中的txt
        if not os.path.exists(temp_path):
            error_msg = "Error. File not found: {}\n".format(temp_path)
            error(error_msg)
        lines_list      = file_lines_to_list(txt_file)      #遍历每个gt的txt，将其中的内容转化成list
                                                            # [['target x1 y1 x2 y2'],['target x1 y1 x2 y2'],......]
        bounding_boxes  = []
        is_difficult    = False
        already_seen_classes = []
        #填充bounding_boxes、already_seen_classes、counter_images_per_class以及gt_couter_per_class
    ...
    gt_classes  = sorted(gt_classes)        #gt_classes = ['target']
    n_classes   = len(gt_classes)           #n_classes = 1

    dr_files_list = glob.glob(DR_PATH + '/*.txt')
    dr_files_list.sort()
    for class_index, class_name in enumerate(gt_classes):       #class_index---> target:0    class_name:'target'
        bounding_boxes = []
...
def get_map_for_train(MINOVERLAP, draw_plot, path='./map_out_IRDST_992_2_1'):
    GT_PATH = os.path.join(path, 'ground-truth')
    DR_PATH = os.path.join(path, 'detection-results')
    IMG_PATH = os.path.join(path, 'images-optional')
    TEMP_FILES_PATH = os.path.join(path, '.temp_files')
    ...
    if os.path.exists(RESULTS_FILES_PATH):
        ...
        with open(os.path.join(path,'results','results.txt'), 'w') as f:
            pass
    if draw_plot:
        os.makedirs(os.path.join(RESULTS_FILES_PATH, "AP"))
        os.makedirs(os.path.join(RESULTS_FILES_PATH, "F1"))

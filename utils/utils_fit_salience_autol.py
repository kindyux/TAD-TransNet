...
def fit_one_epoch_salience(model, yolo_loss, salience_loss, optimizer, epoch, epoch_step, gen, Epoch, cuda, save_dir,
                           best_iou, meta, meta_optimizer, meta_loader, optimizer_current_lr):
    object_loss_total = 0
    ...
    with tqdm(total=epoch_step, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for (iteration, (batch, meta_batch)) in enumerate(zip(gen, meta_loader)):
            ...
            loss_value = meta.meta_weights[0] * object_loss_value + meta.meta_weights[1] * salience_loss_value
    ...
    path = "K:/hongwaixiaomubiaoshujuji_for_TAD/IRSTD-1k/test.txt"
    img_root = 'K:/hongwaixiaomubiaoshujuji_for_TAD/IRSTD-1k/JPEGImages/'
    png_root = 'K:/hongwaixiaomubiaoshujuji_for_TAD/IRSTD-1k/masks/'
    ...
    with torch.no_grad():
        ...
        if current_iou >= best_iou:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir,
                                                        'epoch_{}_IoU_{}_nIoU_{}_PD_{}_FA_{}.pth'.format((epoch + 1),
                                                                                                         round(
                                                                                                             current_iou,
                                                                                                             5), round(
                                                                current_niou, 5), round(current_PD, 5), round(
                                                                current_FA, 5))))
            return current_iou
        else:
            print('之前最好的IoU为：', best_iou, '\n', '当前轮次的IoU为：', current_iou, '\n', '不满足条件!')
